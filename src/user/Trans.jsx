...
export default function CheckoutPage() {
        ...
        if (productId) {
            ...
            if (!isNaN(parsedProductId)) {
                axios.get(`http://localhost:1111/getProductById/${parsedProductId}`)
                    .then(response => {
                        setProductAmount(response.data.price);
    ...
    const handlePayNow = async (e) => {
        ...
        try {
            const parsedProductId = parseInt(productId, 10); // Parse productId as integer
            if (!isNaN(parsedProductId)) {
                if (cardDetails.balance < productAmount) {
                    Swal.fire({
                        title: "Payment Failed!",
                        text: "You have limited amount in the balance. Check your balance before ordering.",
                        icon: "error"
                    });
    ...
    return (
        <div>
            ...
            <Container maxWidth="sm" id="transcon">
